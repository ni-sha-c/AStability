...
for i in range(n_dpts):
    for j in range(i+1, n_dpts):
        ...
        k = k + 1
        ax.semilogy(range(num_el)[::skip], m_dloss[::skip]/1.e-3, color="midnightblue", linewidth=2.0, label='Lyap stab')
...
ax.semilogy(range(0,dacc*nacc,dacc)[::skip], m_dacc[::skip]/1.e-3, color="midnightblue", lw = 3.0, label='Lyap stab')
ax.legend(fontsize=16)
tight_layout()
...
ax.semilogy(range(num_el)[::skip], m_dn[::skip]/1.e-3, color="midnightblue", linewidth=2.0, label='Stoc stab')
ax.legend(fontsize=16)
tight_layout()
...
ax.semilogy(range(num_el)[::skip], m_dc[::skip], color="midnightblue", linewidth=2.0, label='Lyap stab')
ax.legend(fontsize=16)
tight_layout()
